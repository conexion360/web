<script setup>
// Simplificado - eliminando funciones innecesarias y efectos pesados
...
// Función simplificada para abrir enlaces
const openSocial = (url) => {
  window.open(url, '_blank');
};

// Solo mantenemos una función simple para los efectos de scroll
import { onMounted } from 'vue';

onMounted(() => {
  // Implementación simplificada del efecto reveal-on-scroll
...
<template>
  <section id="redes-sociales" class="relative py-24 overflow-hidden">
    <!-- Fondo simplificado con menos elementos y capas -->
    ...
    <div class="container mx-auto px-6 max-w-6xl relative z-10">
      ...
      <div class="text-center mb-16 reveal-on-scroll">
        <h2 class="text-5xl sm:text-6xl font-bold mb-6">
          <span class="text-white">Nuestras</span>
          <span class="text-secondary ml-2">Redes Sociales</span>
        </h2>
        <p class="text-gray-300 max-w-3xl mx-auto text-lg mt-4">
      ...
      <!-- Tarjetas de redes sociales simplificadas -->
      <div class="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
        <div 
          v-for="(network, index) in socialNetworks" 
          :key="network.name"
          class="reveal-on-scroll transition-all duration-500"
          :style="{ 'transition-delay': `${index * 100}ms` }"
          @click="openSocial(network.url)"
        >
          <!-- Tarjeta simplificada con menos capas y efectos -->
          <div class="bg-white/5 hover:bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl p-8 text-center cursor-pointer transition-all duration-300 hover:-translate-y-2 hover:shadow-lg h-full">
            <!-- Icono simplificado -->
            <div class="relative mx-auto mb-6 w-20 h-20">
              <div :class="`${network.color} rounded-full flex items-center justify-center w-20 h-20 transition-transform duration-300 hover:scale-105`">
                <svg 
                  ...
                  fill="currentColor"
                  class="text-white"
                  v-html="network.iconPath"
                ></svg>
              </div>
            </div>
            
            <!-- Contenido simplificado -->
            <h4 class="text-2xl font-bold text-white mb-2">{{ network.name }}</h4>
            <p class="text-gray-400 mb-4">{{ network.username }}</p>
            
            <!-- Botón simplificado -->
            <div class="text-secondary font-medium inline-flex items-center gap-1">
              Seguir
